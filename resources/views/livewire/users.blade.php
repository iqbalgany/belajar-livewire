<div class="flex justify-center gap-10 px-5">
    ...
    <div class="w-1/3 my-10">
        <div class="py-2">
            ...
            @if (session('success'))
                <div class="p-4 mb-4  mt-10 text-sm text-green-800 rounded-lg bg-green-50 " role="alert">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <div class="mt-4">
                <form wire:submit.prevent="createNewUser" action="#" method="POST" class="space-y-6">
                    <div>
                        ...
                        <div class="mt-2">
                            <input wire:model="name" id="name" type="text" name="email" autocomplete="name"
                    ...
                    <div wire:loading wire:target="avatar"
                        class="flex items-center justify-center w-20 h-30 border border-gray-200 rounded-lg bg-gray-50 ">
                        <div
                            class="px-3
                    py-1 text-[10px] font-medium leading-none text-center text-blue-800 bg-blue-200 rounded-full
                    animate-pulse">
                            loading...</div>
                    </div>

                    @if ($avatar)
                        <p class="mmy-2 text-sm font-medium">Preview</p>
                        <img src="{{ $avatar->temporaryUrl() }}" class="rounded w-20 h-30 block object-cover">
                    @endif
                    ...
                    <div>
                        <button wire:click.prevent="createNewUser"
                            ...
                            <svg wire:loading wire:target="createNewUser" aria-hidden="true"
                                class="w-4 h-4 text-gray-200 animate-spin  fill-blue-600 self-center mr-2"
    ...
    <div class="w-132 my-10">
        ...
        <form wire:submit="search" class="max-w-lg mx-auto">
            ...
            <div class="relative">
                ...
                <input wire:model.live.debounce.250="query" type="search" id="default-search"
            ...
            @foreach ($users as $user)
                <li class="flex justify-between gap-x-6 py-5">
                    <div class="flex min-w-0 gap-x-4">
                        <img src="{{ $user->avatar ? asset('storage/' . $user->avatar) : asset('image/image.png') }}"
                    ...
                    <div class="hidden shrink-0 sm:flex sm:flex-col sm:items-end self-center">
                        <p class="mt-1 text-xs/5 text-gray-500">{{ $user->created_at->diffForHumans() }}</time>
                        </p>
                    </div>
                </li>
            @endforeach
